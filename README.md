...
This project is a fork of [Pake](https://github.com/tw93/Pake), originally created by [Tw93](https://github.com/tw93) and contributors. Licensed under the MIT License.

### AppForge Additions

This fork adds the following features on top of the original Pake:

- **Visual GUI App Builder** — React + Tauri desktop app with drag-and-drop interface, window size presets, and build log streaming
- **Batch Generation** (`--batch`) — Build multiple apps from a JSON config file in one run
- **Profile System** (`--save-profile`, `--load-profile`) — Save and reload app configurations as reusable profiles
- **Custom CSS/JS Injection Editor** — Visual editor for injecting styles and scripts into packaged apps
